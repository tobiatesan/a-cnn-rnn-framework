...
class ltwin(object):
    def __init__(self, input_types, input_shapes, m, tau, embeddings_width, config):
        ...
        if (config["arch"] in ["ltwin+rnn", "ltwin+2rnn"]):
            ...
            reducer = lambda outns, foo: tf.reduce_max(outns, axis = 0)

        (featmain, featpool) = make_pipe("feat", fst_feat, nth_feat,
                                         self.dropout, config, reducer, m, h)
        (vecmain, vecpool) = make_pipe("vec", fst_vec, nth_vec,
                                       self.dropout, config, reducer, m, h)

        print(vecpool.shape)
        if (config["arch"] in ["ltwin+2rnn"]):
            inputs = [featmain, featpool, vecmain, vecpool]
            cell = tf.nn.rnn_cell.LSTMCell(81, activation="linear") # HACK
            out, state = tf.nn.static_rnn(cell=cell, inputs=inputs, dtype=tf.float32, scope="OUT")
            self.model = out[-1]
        else:
            conc = tf.concat([featmain, featpool, vecmain, vecpool], axis=1)
            self.model = self.fully = tf.layers.dense(conc,
        ...
        var_list = [v for v in tf.trainable_variables() if
                    v.name.split('/')[0] in (["FCZfeat", "FCZvec", "FCfeat",
                                              "FCvec", "OUT", "rnnfeat", "rnnvec", "RED"])]


        if (not ("unbridle_rnn" in config.keys() and config["unbridle_rnn"])):
            kernels_by_shape = [["FCZfeat/kernel:0", "FCfeat/kernel:0"], ["FCZvec/kernel:0", "FCvec/kernel:0"],
                                ["OUT/kernel:0"], ["rnnvec/lstm_cell/kernel:0"], ["rnnfeat/lstm_cell/kernel:0"], ["rnnfeat/kernel:0"], ["rnnvec/kernel:0"], ["OUT/lstm_cell/kernel:0"]]
        else:
            kernels_by_shape = [["FCZfeat/kernel:0", "FCfeat/kernel:0"], ["FCZvec/kernel:0", "FCvec/kernel:0"],
                                ["OUT/kernel:0"], ["rnnvec/lstm_cell/kernel:0"], ["rnnfeat/lstm_cell/kernel:0"], ["rnnfeat/kernel:0"], ["rnnvec/kernel:0"]]
